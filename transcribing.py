...
# ==========================================
API_KEY = os.environ.get("YANDEX_CLOUD_API_KEY", "AQVN0mjyvg5mAKfId6ZhU-ae6lXllOnj9aX65yon")
FOLDER_ID = os.environ.get("YANDEX_CLOUD_FOLDER", "b1g1dvtf1uq1af6af3ui")

# Настройки бакета
BUCKET_NAME = os.environ.get("YANDEX_STORAGE_BUCKET_NAME", "msu-project-creaai-sessions")
AWS_ACCESS_KEY = os.environ.get("YANDEX_STORAGE_ACCESS_KEY", "YCAJEeZo_9m2TwB4_YkQfeCh")
AWS_SECRET_KEY = os.environ.get("YANDEX_STORAGE_SECRET_KEY", "YCNj5WJh6nKhoSpw-U_RnM8JaYIxdwTyrDw_cIes")
...
def pretty_api_error(response: requests.Response) -> str:
    try:
        return json.dumps(response.json(), ensure_ascii=False, indent=2)
    except Exception:
        return response.text

def parse_stt_json(raw_json_text: str) -> str:
    try:
        ...
        for line in raw_json_text.strip().split("\n"):
            ...
            if not words:
                print(f"[!] Отсутствуют слова в сегменте: {content}")
                continue

            start_ms_raw = words[0].get("startTimeMs")
            end_ms_raw = words[-1].get("endTimeMs")

            if start_ms_raw is None or end_ms_raw is None:
                print(f"[!] Отсутствуют временные метки в сегменте: {content}")
                continue
        ...
        print(f"[!] Ошибка декодирования JSON: {e}. Сырые данные: {raw_json_text[:200]}...")
        return f"Ошибка декодирования JSON: {e}"
    except Exception as e:
        print(f"[!] Неизвестная ошибка при парсинге JSON: {e}. Сырые данные: {raw_json_text[:200]}...")
        return f"Неизвестная ошибка при парсинге JSON: {e}"

def process_audio(local_path: str, s3_key: str) -> str | None:
    ...
    try:
        ...
        if magic.startswith(b"RIFF"):
            ...
            print("[*] Обнаружен WebM. Запускаю конвертацию...")
        ...
        s3_client.upload_file(str(target_path), BUCKET_NAME, s3_key)
        ...
        resp = requests.post("https://stt.api.cloud.yandex.net/stt/v3/recognizeFileAsync",
                              headers=auth_headers, json={"uri": f"https://storage.yandexcloud.net/{BUCKET_NAME}/{s3_key}",
                                                           "recognitionModel": {"model": "general", "audioFormat": fmt, "languageRestriction": {"languageCode": ["ru-RU"]}}}, verify=False, timeout=60)
        ...
        while True:
            time.sleep(5)
            op = requests.get(f"https://operation.api.cloud.yandex.net/operations/{op_id}", headers=auth_headers, verify=False, timeout=60).json()
            if op.get("done"):
                break

        res = requests.get(f"https://stt.api.cloud.yandex.net/stt/v3/getRecognition?operation_id={op_id}", headers=auth_headers, verify=False, timeout=60)
        return res.text
    except Exception as e:
        print(f"[!] Ошибка: {e}")
        return None
...
def scan_and_transcribe():
    ...
    for root, _, files in os.walk(DATA_USERS_DIR):
        for file in files:
